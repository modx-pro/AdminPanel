...
switch ($modx->event->name) {
	...
	case 'OnWebPagePrerender':
		...
		if (($users == '' && $modx->user->hasSessionContext('mgr')) || ($modx->user->id > 0 && in_array($modx->user->id, $users_arr))) {
			if ($templates == '' || !in_array($modx->resource->template, $templates_arr)) {
				...
				$AdminPanel = $modx->getService('adminpanel', 'AdminPanel', $path . 'model/adminpanel/', $scriptProperties);
